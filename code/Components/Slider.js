...
export default function Slider(props) {
    ...
    const sliderLeft = () => {
        ...
        let quantityItemsTrack
        if(widthTrack === 385) {
            quantityItemsTrack = Math.ceil(widthTrack / widthItem)
        } else {
            quantityItemsTrack =  Math.floor(widthTrack / widthItem)
        }
            ...
            <div className="slider-content">
                <div className="slider-wrapper" ref={sliderWidth}>
                    {props.array[0] ? props.array[0].items.map(item=>{
                        return <Slide key={item.kinopoiskId || item.filmId } id={item.kinopoiskId || item.filmId} name={item.nameRu} year={item.year} src={item.posterUrlPreview} sliderHeader={'slider__header-item'} classImg={'slider__header-item-image'}/>
                    }) : null}
                </div>
            </div>
        </div>
